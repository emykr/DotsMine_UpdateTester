...
const dataPath = path.join(sysRoot, '.SeaDriver-Games')
...
/**
 * Get the credit file path
 * 
 * @returns {string} The path to the credit.json file
 */
exports.getCreditPath = function(){
    // 앱의 실제 경로에서 credit.json 파일을 찾도록 수정
    return path.join(__dirname, '../../assets/credit/credit.json')
}

/**
 * Load credit data from credit.json
 * 
 * @returns {Promise<Array>} Array of credit entries
 */
exports.loadCredits = async function(){
    try {
        const creditPath = exports.getCreditPath()
        logger.info('Loading credits from:', creditPath)
        
        const exists = await fs.pathExists(creditPath)
        if(!exists){
            logger.error('Credit file not found at:', creditPath)
            return []
        }
        
        const data = await fs.readJson(creditPath)
